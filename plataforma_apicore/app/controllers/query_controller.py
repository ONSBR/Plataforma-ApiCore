...
class QueryController:
    ...
    def query(self):
        ...
        projection['where'] = self.mapper.transform.get_filters(
            self.app_id, self.mapped_entity, self.query_string)

        if not list(projection["where"].keys()):
            projection.pop('where', None)

        result = self.query_service.filter(
            self.app_id, self.mapped_entity, entity, projection, self.page,
            self.page_size)
